...
export const LiveStatusBadge: React.FC<LiveStatusBadgeProps> = ({
  ...
  if (!mounted || !status) {
    ...
    return (
      <div className={`glass-panel rounded-2xl p-3 sm:p-4 border border-[#C99738]/30 shadow-md text-left ${className}`}>
  ...
  if (compact) {
    return (
      <Link
        href="/timings"
        className={`inline-flex items-center gap-2 px-2.5 sm:px-3 py-1 rounded-full font-bold transition-all shadow-md hover:scale-[1.03] cursor-pointer text-left ${
          isOpen
      ...
      >
        ...
        <span className={`font-semibold tracking-wide truncate ${language === 'ml' ? 'text-[11px] font-malayalam-sans' : 'text-xs'}`}>
          {statusText[language]}
        </span>
      </Link>
    );
  ...
  return (
    <div
      className={`glass-panel rounded-2xl p-3 sm:p-4 md:px-5 md:py-3.5 border transition-all shadow-md text-left ${
        isOpen
    ...
    >
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-left w-full">
        {/* Left Side: Icon + Live Dot + Left-aligned Status & SubText */}
        <div className="flex items-center gap-3 text-left w-full sm:w-auto flex-1 min-w-0">
          <div
          ...
          <div className="text-left flex-1 min-w-0">
            <div className="flex items-center gap-2 text-left">
              ...
              />
              <h4
                className={`font-bold text-[#1A0409] text-left truncate sm:overflow-visible ${
                  language === 'ml'
                    ? 'text-xs sm:text-sm font-malayalam-sans leading-tight'
                    : 'text-xs sm:text-sm md:text-base font-cinzel leading-tight'
                }`}
              >
                {statusText[language]}
              </h4>
            ...
            {showDetail && (
              <p
                className={`text-[#5A382A] text-left mt-0.5 font-medium truncate sm:overflow-visible ${
                  language === 'ml'
                    ? 'text-[11px] sm:text-xs font-malayalam-sans leading-tight'
        ...
        {/* Right Side: Clickable Button Linking to /timings */}
        <div className="flex items-center justify-end flex-shrink-0 w-full sm:w-auto">
          <Link
            href="/timings"
            className="inline-flex items-center gap-1.5 text-[10px] sm:text-[11px] uppercase tracking-wider font-bold text-[#8C6219] hover:text-[#FAF5E8] bg-[#FAF5E8] hover:bg-[#610C1B] border border-[#E4D5AE] hover:border-[#610C1B] px-3.5 py-1.5 rounded-full shadow-xs hover:shadow-md transition-all cursor-pointer group whitespace-nowrap"
          >
            <span
              className={
                language === 'ml'
                  ? 'text-[11px] font-malayalam-sans font-semibold'
